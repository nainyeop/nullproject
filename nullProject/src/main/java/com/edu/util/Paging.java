...
public class Paging implements Serializable{
	...
	public Paging(int count, int curPage, int blockScale, int pageScale) {
		curBlock = 1;   // 현재 페이지 블록 번호
		this.curPage = curPage;  // 현재 페이지 설정
		setBlockScale(blockScale); // 화면당 페이지 수
		setPageScale(pageScale); // 페이지당 게시물 수
		setTotPage(count);  // 전체 페이지 갯수 계산
		setPageRange();
		setTotBlock();  // 전체 페이지 블록 갯수 계산
		setBlockRange();  // 페이지 블록의 시작, 끝 번호
	...
	public void setBlockRange() {
		...
		nextPage = curBlock > totBlock ? (curBlock * blockScale)
				: (curBlock * blockScale) + 1;
